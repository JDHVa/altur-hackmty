...
export async function saveCall(input: SaveCallInput) {
  ...
  const [row] = await db
    ...
    .values({
      ...
      pWavlm: input.final.signals?.wavlm ?? null,
      pXlsr: input.final.signals?.xlsr ?? null,
      pFlow: input.final.signals?.flow ?? null,
  ...
  if (input.frames.length) {
    await db.insert(schema.callScores).values(
      input.frames.map((f) => ({
        ...
        pFinal: f.p_final,
        pWavlm: f.signals?.wavlm ?? null,
        pXlsr: f.signals?.xlsr ?? null,
        pFlow: f.signals?.flow ?? null,
        hnr: f.bio.hnr ?? null,
